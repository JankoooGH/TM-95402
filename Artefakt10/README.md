...
* **Wniosek:** 
* Docker eliminuje problem „u mnie działa" – środowisko jest identyczne na każdej maszynie, wersje narzędzi są przypięte, a po testach kontenery znikają bez śladu w systemie.
...
* **Wniosek:**
* MobSF pozwala zbadać APK bez uruchamiania aplikacji – ujawnia nadmiarowe uprawnienia, hardkodowane klucze API i nieszyfrowane dane zanim napisze się pierwszy test UI.

### 🔹 BLOK 3-4: Fundamenty Skryptowania (Python for QA)
Budowa logiki testowej w języku Python.
* **Co zrobiono:**
* Omówiono kluczowe struktury danych Pythona (listy, słowniki, krotki), definiowanie funkcji, obsługę wyjątków (try/except) oraz pracę z plikami JSON – czyli zestaw niezbędny do pisania logiki testowej.
...
* **Co zrobiono:** 
* Używano selektorów resource-id (szybkie, stabilne) oraz XPath (złożone hierarchie). Symulowano: tap, wprowadzanie tekstu (send_keys), swipe oraz oczekiwanie na elementy przez WebDriverWait.
